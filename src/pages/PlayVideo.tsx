import { ChevronLeft, ChevronRight, ListEnd, RotateCcw } from "lucide-react";
  ...
  return (
    <div className="min-h-screen dark:bg-[#0d0714]">
      <div className="flex flex-col items-center py-10 px-4 max-w-6xl mx-auto">
        <h1 className="dark:text-white text-center text-xl sm:text-3xl font-bold mb-4">{video.snippet.title}</h1>
        <p className="dark:text-white/70 mb-6">{video.snippet.channelTitle}</p>
        ...
        <div className="flex gap-4">
          <button
            ...
            className="px-4 py-2 cursor-pointer active:scale-95 transition-all flex flex-row gap-2 bg-black/10 dark:bg-white/10 dark:text-white rounded-full dark:hover:bg-white/20"
          >
            <ListEnd /> Back to Playlist
          </button>
          <button
            onClick={changeMood}
            className="px-4 py-2 cursor-pointer active:scale-95 transition-all flex flow-row gap-2 bg-black/10  dark:bg-white/10 dark:text-white rounded-full dark:hover:bg-white/20"
          >
